...
const ReverseFly = () => {
    ...
    useEffect(() => {
        const storedItems = localStorage.getItem('Reverse-Fly') || [];
        setItems(storedItems);
      }, []);
  
    const handleSave = event => {
        const date = dayjs().format('MMM D, YYYY')
        localStorage.setItem('Reverse-Fly', `${value +  ' pounds on '  + date}`)
        window.location.href = '/';
    }
  
    return (
        <div className='p-2 min-h-screen bg-no-repeat bg-contain font-semibold' style={{ backgroundImage: `url(${ReverseFlysImg})` }}>
            <div className='bg-black/50 p-2'>
                <h1 className='text-2xl'>Reverse Fly</h1>
