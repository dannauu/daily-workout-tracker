...
const MachineShoulderPress = () => {
    ...
    useEffect(() => {
        const storedItems = localStorage.getItem('Machine-Shoulder-Press') || [];
        setItems(storedItems);
      }, []);
  
    const handleSave = event => {
        const date = dayjs().format('MMM D, YYYY')
        localStorage.setItem('Machine-Shoulder-Press', `${value +  ' pounds on '  + date}`)
        window.location.href = '/';
    }
  
    return (
        <div className='p-2 min-h-screen bg-no-repeat bg-contain font-semibold' style={{ backgroundImage: `url(${MachineShoulderPressImg})` }}>
            <div className='bg-black/50 p-2'>
                <h1 className='text-2xl'>Machine Shoulder Press</h1>
...
export default MachineShoulderPress