...
const Shrugs = () => {
    ...
    useEffect(() => {
        const storedItems = localStorage.getItem('Shrugs') || [];
        setItems(storedItems);
      }, []);
  
    const handleSave = event => {
        const date = dayjs().format('MMM D, YYYY')
        localStorage.setItem('Shrugs', `${value +  ' pounds on '  + date}`)
        setShowMessage(true);
        setTimeout(() => {
    ...
    const markDone = event => {
        localStorageHelper('done', 'shrugs')
        window.location.href = '/thursday'
    }
  
    return (
        <div className='p-2 h-96 bg-contain bg-no-repeat font-semibold' style={{ backgroundImage: `url(${ShrugsImg})` }}>
            <div className='bg-black/50 p-2'>
                <h1 className='text-2xl'>Shrugs</h1>
