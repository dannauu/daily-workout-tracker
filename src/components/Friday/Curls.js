...
const Curls = () => {
    ...
    useEffect(() => {
        const storedItems = localStorage.getItem('Curls') || [];
        setItems(storedItems);
      }, []);
  
    const handleSave = event => {
        const date = dayjs().format('MMM D, YYYY')
        localStorage.setItem('Curls', `${value +  ' pounds on '  + date}`)
        window.location.href = '/';
    }
  
    return (
        <div className='p-2 min-h-screen bg-no-repeat bg-contain font-semibold' style={{ backgroundImage: `url(${StandingCurlsImg})` }}>
            <div className='bg-black/50 p-2'>
                <h1 className='text-2xl'>Curls</h1>
