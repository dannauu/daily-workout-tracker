...
const MachineChestPress = () => {
    ...
    useEffect(() => {
        const storedItems = localStorage.getItem('Machine-Chest-Press') || [];
        setItems(storedItems);
      }, []);

    const handleSave = event => {
        const date = dayjs().format('MMM D, YYYY')
        localStorage.setItem('Machine-Chest-Press', `${value +  ' pounds on '  + date}`)
        setShowMessage(true);
        setTimeout(() => {
    ...
    const markDone = event => {
        localStorageHelper('done', 'machine-chest-press')
        window.location.href = '/tuesday'
    }

    return (
        <div className='p-2 h-96 bg-contain bg-no-repeat font-semibold' style={{ backgroundImage: `url(${MachineChestPressImg})` }}>
            <div className='bg-black/50 p-2'>
                <h1 className='text-2xl'>Machine Chest Press</h1>
