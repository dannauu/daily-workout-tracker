...
const DumbbellChestFly = () => {
    ...
    useEffect(() => {
        const storedItems = localStorage.getItem('Dumbbell-Chest-Fly') || [];
        setItems(storedItems);
      }, []);

    const handleSave = event => {
        const date = dayjs().format('MMM D, YYYY')
        localStorage.setItem('Dumbbell-Chest-Fly', `${value +  ' pounds on '  + date}`)
        setShowMessage(true);
        setTimeout(() => {
    ...
    return (
        <div className='p-2 h-96 bg-contain bg-no-repeat font-semibold' style={{ backgroundImage: `url(${DumbbellChestFlyImg})` }}>
            <div className='bg-black/50 p-2'>
                <h1 className='text-2xl'>Dumbbell Chest Fly</h1>
...
export default DumbbellChestFly