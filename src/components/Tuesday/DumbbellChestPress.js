...
const DumbbellChestPress = () => {
    ...
    useEffect(() => {
        const storedItems = localStorage.getItem('Dumbbell-Chest-Press') || [];
        setItems(storedItems);
      }, []);

    const handleSave = event => {
        const date = dayjs().format('MMM D, YYYY')
        localStorage.setItem('Dumbbell-Chest-Press', `${value +  ' pounds on '  + date}`)
        setShowMessage(true);
        setTimeout(() => {
    ...
    const markDone = event => {
        localStorageHelper('done', 'dumbbell-chest-press')
        window.location.href = '/tuesday'
    }


    return (
        <div className='p-2  h-96 bg-contain bg-no-repeat font-semibold' style={{ backgroundImage: `url(${DumbbellChestFlyImg})` }}>
            <div className='bg-black/50 p-2'>
                <h1 className='text-2xl'>Dumbbell Chest Press</h1>
...
export default DumbbellChestPress