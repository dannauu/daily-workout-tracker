...
const InclineBench = () => {
    ...
    useEffect(() => {
        const storedItems = localStorage.getItem('Incline-Bench') || [];
        setItems(storedItems);
      }, []);

    const handleSave = event => {
        const date = dayjs().format('MMM D, YYYY')
        localStorage.setItem('Incline-Bench', `${value +  ' pounds on '  + date}`)
        setShowMessage(true);
        setTimeout(() => {
    ...
    const markDone = event => {
        localStorageHelper('done', 'incline-bench')
        window.location.href = '/tuesday'
    }

    return (
        <div className='p-2  h-96 bg-contain bg-no-repeat font-semibold' style={{ backgroundImage: `url(${InclineBenchImg})` }}>
            <div className='bg-black/50 p-2'>
                <h1 className='text-2xl'>Incline Bench</h1>
