...
const Lunges = () => {
  ...
  useEffect(() => {
      const storedItems = localStorage.getItem('Lunges') || [];
      setItems(storedItems);
    }, []);

  const handleSave = event => {
      const date = dayjs().format('MMM D, YYYY')
      localStorage.setItem('Lunges', `${value +  ' pounds on '  + date}`)
      window.location.href = '/';
  }

  return (
      <div className='p-2 min-h-screen bg-no-repeat bg-contain font-semibold' style={{ backgroundImage: `url(${LungesImg})` }}>
          <div className='bg-black/50 p-2'>
              <h1 className='text-2xl'>Lunges</h1>
