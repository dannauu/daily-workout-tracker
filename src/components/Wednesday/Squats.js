...
const Squats = () => {
  ...
  useEffect(() => {
      const storedItems = localStorage.getItem('Squats') || [];
      setItems(storedItems);
    }, []);

  const handleSave = event => {
      const date = dayjs().format('MMM D, YYYY')
      localStorage.setItem('Squats', `${value +  ' pounds on '  + date}`)
      window.location.href = '/';
  }

  return (
      <div className='p-2 min-h-screen bg-no-repeat bg-contain font-semibold' style={{ backgroundImage: `url(${SquatImg})` }}>
          <div className='bg-black/50 p-2'>
              <h1 className='text-2xl'>Squats</h1>
