...
const Wallsit = () => {
  ...
  useEffect(() => {
      const storedItems = localStorage.getItem('Wallsit') || [];
      setItems(storedItems);
    }, []);

  const handleSave = event => {
      const date = dayjs().format('MMM D, YYYY')
      localStorage.setItem('Wallsit', `${value +  ' pounds on '  + date}`)
      window.location.href = '/';
  }

  return (
      <div className='p-2 min-h-screen bg-no-repeat bg-contain font-semibold' style={{ backgroundImage: `url(${WallsitImg})` }}>
          <div className='bg-black/50 p-2'>
              <h1 className='text-2xl'>Wallsit</h1>
